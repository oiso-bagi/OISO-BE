...
이 문서는 OISO-BE에서 Codex config를 어디에 둘지 정의합니다.
...
OISO-BE의 팀 공통 기준은 Codex config가 아니라 저장소 파일과 문서에서 확인합니다.
...
DB 접속 정보, API key, token, 로컬 secret은 repo에 커밋하지 않습니다. 로컬 실행에는 `.env`를 사용할 수 있지만, agent는 `.env` 값을 읽거나 출력하지 않습니다. 예시가 필요하면 secret 값 없이 `.env.example`만 둡니다.
