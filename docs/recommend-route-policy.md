...
| --- | --- | --- |
| **루트 기본 단위 (Route Unit)** 🆕 | **1일차(1-Day) 코스 모듈** | 모든 코스의 생성/저장 기본 단위는 1일 코스이며, N일 여행 시 런타임에 1일 모듈 N개를 체이닝 조합 |
...
- **예산 범위 및 권장 안내 (Budget Range Guidance)**: DB 내 사전 적재된 마스터 코스의 1일치 최저 예상 비용은 25,000원이므로, 클라이언트 요청 시 1일 예산(`dailyBudgetWon`)을 25,000원 이상으로 설정할 때 가장 다양하고 풍부한 추천 코스 결과를 제공받을 수 있습니다.
