...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description: [
      ...
      '3. 사용자가 카카오에서 인증을 완료하면 카카오가 GET /api/v1/auth/kakao/callback 경로를 호출합니다.',
      '4. 서버는 인증 코드와 state를 검증하고 리프레시 토큰 쿠키를 설정합니다.',
      '5. 처리 결과에 따라 프론트엔드의 성공, 약관 동의, 또는 실패 화면으로 리다이렉트합니다.',
      '6. 프론트엔드는 로그인 성공 화면 진입 후 POST /api/v1/auth/refresh를 호출해 액세스 토큰을 발급받을 수 있습니다.',
    ].join('\n'),
  })
  @ApiFoundResponse({
    description:
      '카카오 OAuth 인증 URL로 리다이렉트합니다. 프론트엔드는 이 API를 AJAX로 호출하기보다 브라우저 이동 방식으로 연결하는 것을 권장합니다.',
  })
  @ApiInternalServerErrorResponse({
    description:
  ...
  @ApiOperation({
    ...
    description: [
      ...
      '7. 프론트엔드는 로그인 성공 화면 진입 후 POST /api/v1/auth/refresh를 호출해 액세스 토큰을 발급받을 수 있습니다.',
    ].join('\n'),
  })
  @ApiFoundResponse({
    description: [
  ...
  @ApiOperation({
    ...
    description: [
      ...
      '3. 사용자가 구글에서 인증을 완료하면 구글이 GET /api/v1/auth/google/callback 경로를 호출합니다.',
      '4. 서버는 인증 코드와 state를 검증하고 리프레시 토큰 쿠키를 설정합니다.',
      '5. 처리 결과에 따라 프론트엔드의 성공, 약관 동의, 또는 실패 화면으로 리다이렉트합니다.',
      '6. 프론트엔드는 로그인 성공 화면 진입 후 POST /api/v1/auth/refresh를 호출해 액세스 토큰을 발급받을 수 있습니다.',
    ].join('\n'),
  ...
  @ApiFoundResponse({
    description:
      '구글 OAuth 인증 URL로 리다이렉트합니다. 프론트엔드는 이 API를 AJAX로 호출하기보다 브라우저 이동 방식으로 연결하는 것을 권장합니다.',
  })
  @ApiInternalServerErrorResponse({
    description:
  ...
  @ApiInternalServerErrorResponse({
    ...
    content: {
      'application/json': {
        examples: {
          jwtAccessSecretMissing: internalServerErrorExamples.jwtSecretMissing,
        },
      },
    },
  })
